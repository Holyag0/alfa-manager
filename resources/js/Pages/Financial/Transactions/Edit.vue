    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div class="bg-white shadow rounded-lg p-6 mb-6">
        <div class="flex items-center justify-between">
          ...
          <Link :href="route('financial.transactions.show', transaction.id)" 
      ...
      <div v-if="transaction.source_type" class="bg-red-50 border-l-4 border-red-400 p-4 mb-6">
        <div class="flex">
          ...
          <div class="ml-3">
            <h3 class="text-sm font-medium text-red-800">Transação Rastreada Automaticamente</h3>
            <div class="mt-2 text-sm text-red-700">
              <p>
                Esta transação foi registrada automaticamente pelo sistema e <strong>não pode ser editada</strong>.
              </p>
            </div>
          </div>
        </div>
      ...
      <!-- Formulário (apenas se for transação manual) -->
      <form v-if="!transaction.source_type" @submit.prevent="submit" class="bg-white shadow rounded-lg p-6">
        <div class="space-y-6">
          <!-- Tipo de Transação (desabilitado) -->
          <div>
            ...
            <div class="px-4 py-3 bg-gray-50 rounded-lg border border-gray-200">
              ...
              <p class="text-xs text-gray-500 mt-2">O tipo de transação não pode ser alterado após a criação</p>
            </div>
          </div>
          ...
          <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-sm font-medium text-gray-900">
                {{ transaction.type === 'receita' ? 'Informações do Pagante' : 'Informações do Fornecedor' }}
              </h3>
              <div class="flex items-center gap-2">
                ...
                <Link :href="route('financial.suppliers.create')" 
                  ...
                  Novo {{ transaction.type === 'receita' ? 'Pagante' : 'Fornecedor' }}
                </Link>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <!-- Nome (Receita) -->
              <div v-if="transaction.type === 'receita'">
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  Nome do Pagante
                </label>
              ...
              <!-- Nome (Despesa) -->
              <div v-if="transaction.type === 'despesa'">
                <label class="block text-sm font-medium text-gray-700 mb-2">
                  Nome do Fornecedor
                </label>
            ...
            <div v-if="(transaction.type === 'receita' && form.payer_supplier_id) || (transaction.type === 'despesa' && form.payee_supplier_id)" 
              ...
              <div class="flex items-center justify-between">
                <div class="flex items-center">
                  ...
                  <div>
                    ...
                    <p v-if="transaction.type === 'receita' && pagantes.find(p => p.id == form.payer_supplier_id)?.document" 
                       class="text-xs text-indigo-600">
                      {{ pagantes.find(p => p.id == form.payer_supplier_id)?.document }}
                    </p>
                    <p v-if="transaction.type === 'despesa' && fornecedores.find(f => f.id == form.payee_supplier_id)?.document" 
                       class="text-xs text-indigo-600">
            ...
            <div class="flex">
              ...
              <div class="ml-3">
                <p class="text-sm text-blue-700">
                  Esta transação está <strong>confirmada</strong> e o status não pode ser alterado para pendente.
                </p>
              </div>
            </div>
          </div>
      ...
      <!-- Mensagem alternativa se for transação rastreada -->
      <div v-else class="bg-white shadow rounded-lg p-6">
        <div class="text-center py-12">
          ...
          <h3 class="text-lg font-medium text-gray-900 mb-2">Transação Bloqueada para Edição</h3>
          <p class="text-sm text-gray-600 max-w-lg mx-auto mb-6">
            Esta transação foi gerada automaticamente pelo sistema e não pode ser editada manualmente.
          </p>
      ...
      <SupplierSearchModal
        ...
        :suppliers="transaction.type === 'receita' ? pagantes : fornecedores"
...
const form = useForm({
  ...
  transaction_date: props.transaction.transaction_date?.split('T')[0] || props.transaction.transaction_date,
  payment_method: props.transaction.payment_method || '',
  reference: props.transaction.reference || '',
  status: props.transaction.status,
  notes: props.transaction.notes || '',
  payer_name: props.transaction.payer_name || '',
  payer_document: props.transaction.payer_document || '',
  payee_name: props.transaction.payee_name || '',
  payee_document: props.transaction.payee_document || '',
  payer_supplier_id: props.transaction.payer_supplier_id || '',
  payee_supplier_id: props.transaction.payee_supplier_id || ''
})

const clearSupplier = () => {
  if (props.transaction.type === 'receita') {
...
const filteredCategories = computed(() => {
  return props.categories.filter(cat => cat.type === props.transaction.type)
})
...
const submit = () => {
  if (props.transaction.source_type) {
    alert('Transações rastreadas automaticamente não podem ser editadas!')
    return
  }
