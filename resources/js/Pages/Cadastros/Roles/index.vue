          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-xs font-medium tracking-wider text-left text-gray-500 uppercase">Permissões</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="role in roles.data" :key="role.id" class="hover:bg-gray-50">
                ...
                <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">{{ role.name }}</td>
                <td class="px-6 py-4 text-sm text-gray-500">
                  <span v-if="role.permissions.length > 0" class="inline-flex items-center px-3 py-1 text-sm font-medium text-blue-800 bg-blue-100 rounded-full">
                    {{ role.permissions.length }} permissões
                  </span>
                  <span v-else class="inline-flex items-center px-3 py-1 text-sm font-medium text-gray-600 bg-gray-100 rounded-full">
                    Nenhuma permissão
                  </span>
                </td>
      ...
      <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              ...
              <div class="mt-2">
                <p class="text-sm text-gray-500">
                  Tem certeza que deseja excluir o papel "{{ roleToDelete ? roleToDelete.name : '' }}"?
          ...
          <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
            <Link
              :href="route('roles.destroy', roleToDelete ? roleToDelete.id : '')"
              method="delete"
              as="button"
