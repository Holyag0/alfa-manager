...
const principalPhoneIndex = ref(0);
const principalContactIndex = ref(0);
const principalAddressIndex = ref(0);

function addPhone() {
  newGuardian.phones.push({ value: '', type: 'pessoal' });
}
function removePhone(idx) {
  newGuardian.phones.splice(idx, 1);
  if (principalPhoneIndex.value >= newGuardian.phones.length) {
    principalPhoneIndex.value = 0;
  }
}

watch(principalPhoneIndex, (idx) => {
  // Garantir que só um é principal
  newGuardian.phones.forEach((p, i) => {
    p.is_primary = (i === idx);
  });
});

function submitNewGuardian() {
  // Marcar o principal antes de enviar
  newGuardian.phones.forEach((p, i) => {
    p.is_primary = (i === principalPhoneIndex.value);
  });
  router.post(route('guardian.store'), newGuardian, {
    preserveState: true,
    replace: true,
    onSuccess: (page) => {
      if (page.props.guardian) {
        emit('next', page.props.guardian);
      }
    },
    onError: (errors) => {
